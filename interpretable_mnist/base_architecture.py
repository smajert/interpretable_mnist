...
class SimpleConvNetRoot(torch.nn.Module):
    ...
    def __init__(
        ...
        self.layers = torch.nn.ModuleList([                     # input: 1 x 28 x 28
            ...
            torch.nn.BatchNorm2d(32) if do_batch_norm else torch.nn.Identity(),
            self.activation,
            torch.nn.Dropout(p=dropout_probs[0]),
            torch.nn.Conv2d(32, 128, 2, stride=2, padding=0),   # 128 x 7 x 7
            torch.nn.BatchNorm2d(128) if do_batch_norm else torch.nn.Identity(),
            self.activation,
            torch.nn.Dropout(p=dropout_probs[1]),
            torch.nn.Conv2d(128, 256, 2, stride=2, padding=0),  # 256 x 3 x 3
            torch.nn.BatchNorm2d(256) if do_batch_norm else torch.nn.Identity(),
            ...
            torch.nn.BatchNorm2d(64) if do_batch_norm else torch.nn.Identity(),
            self.activation,
        ])
        # fmt: on
